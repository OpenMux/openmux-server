...
A `filter_set` has six glob lists (case-sensitive, `*` wildcards): `include` / `exclude` (port name), `adapter_include` / `adapter_exclude` (adapter type), and `server_include` / `server_exclude` (origin server id; meaningfully the `accept` direction). An **empty or missing list means "no constraint"** on that dimension, and exclude patterns win over include. **The default is deny-all (ticket #77):** a direction with no `include`, `adapter_include`, or `server_include` entry matches no port, so by default nothing is shared or accepted. Set `include: ["*"]` to share or accept everything, or `exclude: ["*"]` to block a direction. Filter changes take effect on a Soft Reload. The server logs a warning for each unconstrained direction, once per deny period (a warning does not repeat until the include list changes back to empty).
