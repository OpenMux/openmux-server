  ...
  let rows=[]; let editingIndex=null; function setRows(newRows){
      ...
      const COLUMN_HELP = {
        ...
        'command_ports': {
          ...
          max_read_write_users: 'How many users may write at once: one = 1 (default), multiple = unlimited, none = no driver (admin included).',
          interactive: 'Preset for use_pty, always_buffer, and normalize_newlines. Does not modify the command string and never adds a shell.',
          ...
          auto_restart: 'Restart the process automatically if it exits (useful for long-running daemons that crash and should self-heal).',
          restart_delay: 'Initial delay in seconds before the first restart attempt.',
          max_restarts: 'Maximum restart attempts before giving up. 0 = unlimited.',
          restart_backoff: 'Exponential backoff multiplier applied to the restart delay between consecutive attempts.',
          clean_env: 'Start with a minimal sanitized environment (PATH, HOME, SHELL, USER, LANG, LC_ALL). Merge extra values via the env: block.',
          local_echo: 'Echo client writes back through the output buffer to all connected clients (handy for non-PTY pipes that do not echo).',
          output_crlf: 'Convert LF to CRLF in process output before forwarding to clients, so classic terminal UIs (telnet, Windows console) render correctly.',
          intercept_term_queries: 'Intercept XTGETTCAP terminal capability queries and respond "unsupported" so editor probe timeouts do not stall the session.',
          pty_force_raw: 'Force raw mode on the PTY slave at spawn. Generally not needed; most TUIs set it themselves.',
          pty_enter_mode: 'Input newline mapping for PTY input. none (default) = leave as-is; cr/lf/crlf = normalize to the given sequence.',
          enable_output_batching: 'Batch PTY/process output before forwarding to clients, reducing network chatter for chatty outputs.',
          output_batch_size: 'Max bytes to accumulate before an immediate output flush. Idle and force-flush timeouts also apply.',
          output_batch_timeout: 'Inactivity timeout in seconds before flushing a partial output batch.',
          output_force_flush_timeout: 'Absolute cap in seconds that forces a flush regardless of batch size, so a trickle of bytes still reaches the client.',
          enable_batching: 'Batch writes from clients to the subprocess stdin, reducing syscall overhead for high-fan-in keyboards.',
          batch_size: 'Max bytes to accumulate before flushing a write batch to the process.',
          batch_timeout: 'Max time to wait before flushing a write batch (seconds).',
          scrollback_size: 'Bytes of recent output to buffer for scrollback replay (0 = disabled). Clients request replay with ?scrollback=1.',
          read_write_groups: 'Console groups granted read-write access. Empty = open to all authenticated users.',
          read_only_groups: 'Console groups granted read-only access (never promoted to read-write). Empty = open to all authenticated users.'
        },
      ...
      function initStatic(){
        ...
        tables['command_ports'] = buildTable('command_ports', annotateColumnsWithDefaults('command_ports', annotateColumnsWithHelp('command_ports', [
          ...
          {key:'auto_restart', label:'Auto restart', type:'boolean', hiddenList:true},
          {key:'restart_delay', label:'Restart delay (s)', type:'number', min:0, step:0.1, hiddenList:true},
          {key:'max_restarts', label:'Max restarts', type:'integer', min:0, hiddenList:true},
          {key:'restart_backoff', label:'Restart backoff', type:'number', min:0, step:0.1, hiddenList:true},
          // Environment and terminal behavior (advanced).
          {key:'clean_env', label:'Clean env', type:'boolean', hiddenList:true},
          {key:'local_echo', label:'Local echo', type:'boolean', hiddenList:true},
          {key:'output_crlf', label:'Output CRLF', type:'boolean', hiddenList:true},
          {key:'intercept_term_queries', label:'Intercept XTGETTCAP', type:'boolean', hiddenList:true},
          {key:'pty_force_raw', label:'PTY force raw', type:'boolean', hiddenList:true},
          {key:'pty_enter_mode', label:'PTY enter mode', type:'enum', enum:['none', 'cr', 'lf', 'crlf'], hiddenList:true},
          // I/O tuning (advanced).
          {key:'enable_output_batching', label:'Output batching', type:'boolean', hiddenList:true},
          {key:'output_batch_size', label:'Output batch size (bytes)', type:'integer', min:1, hiddenList:true},
          {key:'output_batch_timeout', label:'Output batch timeout (s)', type:'number', min:0, step:0.001, hiddenList:true},
          {key:'output_force_flush_timeout', label:'Output force flush (s)', type:'number', min:0, step:0.1, hiddenList:true},
          {key:'enable_batching', label:'Write batching', type:'boolean', hiddenList:true},
          {key:'batch_size', label:'Write batch size (bytes)', type:'integer', min:1, hiddenList:true},
          {key:'batch_timeout', label:'Write batch timeout (s)', type:'number', min:0, step:0.001, hiddenList:true},
          {key:'scrollback_size', label:'Scrollback (bytes)', type:'integer', min:0},
          {key:'read_write_groups', label:'RW groups', type:'array-string', hiddenList:true},
          {key:'read_only_groups', label:'RO groups', type:'array-string', hiddenList:true}
        ])));
        ...
        tables['tcp_initiator_ports'] = buildTable('tcp_initiator_ports', annotateColumnsWithDefaults('tcp_initiator_ports', annotateColumnsWithHelp('tcp_initiator_ports', [
          ...
          {key:'ssl_verify', label:'Verify TLS', type:'boolean', hiddenList:true},
          {key:'timeout', label:'Timeout (s)', type:'number', min:0, step:0.1, hiddenList:true},
          {key:'auto_reconnect', label:'Auto reconnect', type:'boolean'},
          {key:'reconnect_delay', label:'Reconnect delay (s)', type:'number', min:0, step:0.1, hiddenList:true},
          {key:'enable_batching', label:'Batch writes', type:'boolean', hiddenList:true},
          {key:'batch_size', label:'Batch size (bytes)', type:'integer', min:1, hiddenList:true},
          {key:'batch_timeout', label:'Batch timeout (s)', type:'number', min:0, step:0.001, hiddenList:true},
